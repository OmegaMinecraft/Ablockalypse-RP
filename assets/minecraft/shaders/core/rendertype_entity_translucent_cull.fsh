...
void main() {
    ...
    if(isGUI == 1) {
        if((alpha == 254 || alpha == 140) && zpos > 100.0) discard; // Handled as inventory slot
        if((alpha == 253 || alpha == 139) && zpos < 100.0) discard; // player doll, so discard the icon
    }else{
        if(alpha == 253 || alpha == 139) discard; // discard the icon outside of gui
    }
